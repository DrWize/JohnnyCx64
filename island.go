...
func islandCycleHoliday() string {
	if islandHolidayOverride < 1 || islandHolidayOverride >= 4 {
		if islandHolidayOverride == 4 {
			islandHolidayOverride = -1
		} else {
			islandHolidayOverride = 1
		}
	} else {
		islandHolidayOverride++
	}
	...
	return islandHolidayLabel()
}

func islandRefreshHolidayLayer() {
...
func islandHolidayLabel() string {
	if islandHolidayOverride == -1 {
		return "Automatic (calendar)"
	}
	labels := [...]string{"Off", "Halloween", "St. Patrick", "Christmas", "New Year"}
	if islandHolidayOverride >= 0 && islandHolidayOverride < len(labels) {
		return labels[islandHolidayOverride]
	}
	return "Off"
}
...
func islandDayNightShortLabel() string {
	switch islandDayNightOverride {
	case 0:
		return "Day"
	case 1:
